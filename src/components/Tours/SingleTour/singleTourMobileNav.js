...
export default function SingleTourMobileNav({ singleTourMenu, items }) {
  ...
  return (
    <MobileNavWrapper>
      <OpenButton type="button" aria-label="Toggle Menu" onClick={onToggleNav}>
        <Image
          src={items.hamburgerIconGreen.sourceUrl}
          alt={items.hamburgerIconGreen.altText}
          width={50}
          height={50}
        />
      </OpenButton>
      <div
        css={[
          tw`fixed pt-4 top-0 left-0 z-10 h-full w-full transform bg-primary-dark duration-300 ease-in-out h-160`,
